...
info "Iniciando análisis automático del sistema..."

SYSTEM_PATH=""
DATA_PATH=""

for vol in /Volumes/*; do
    if [ -d "$vol" ] && [ "$vol" != "/Volumes/Recovery" ]; then
...
check_previous_run() {
    ...
    if [ $ya_hecho -gt 0 ]; then
        echo ""
        warn "El bypass ya ha sido aplicado previamente en este equipo ($ya_hecho/3 indicadores detectados)."
        info "Cancelando ejecución para evitar duplicados. Saliendo..."
        exit 0
    fi
}
...
echo ""
info "Equipo limpio detectado. Iniciando Bypass automáticamente en:"
for i in {5..1}; do
    echo -ne "${YEL}$i... ${NC}"
    sleep 1
done
...
info "Creando cuenta de administrador local ($username)..."
...
info "Configurando redirecciones en el archivo hosts..."
for domain in deviceenrollment.apple.com mdmenrollment.apple.com ipprofiles.apple.com; do
    if ! grep -q "$domain" "$hosts_file" 2>/dev/null; then
        echo "0.0.0.0 $domain" >> "$hosts_file"
    fi
done

config_path="$SYSTEM_PATH/private/var/db/ConfigurationProfiles/Settings"
...
echo -e "${GRN}Credenciales: Usuario: Mac | Clave: 1234${NC}"
echo ""

info "Reiniciando equipo en 3 segundos..."
sleep 3
reboot
